...
编辑 `site/config/_default/params.yml` 中的以下字段：
...
将图片、资源文件放入 `static/` 目录，推荐结构见 `AGENTS.md`：

- `static/images/site/` — 站点主题资源（头图、图标、光标）
- `static/images/covers/paccha/` — 文章封面图
- `static/images/avatar/` — 头像
- 文章正文图片优先放在 `content/post/<文章名>/` 与 `index.md` 同目录（Page Bundle）
